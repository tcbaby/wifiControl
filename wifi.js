...
(async () => {
    ...
    await page.waitForTimeout(1000);
    await page.evaluate(() => document.getElementsByTagName('frame')[1].contentWindow.document.getElementById('net-wlan11ac').click())
    await page.waitForTimeout(1000);

    const getWifiStatus = async () => {
        let flag = await page.evaluate(() => document.getElementsByTagName('frame')[1].contentWindow.document.getElementById('wlacInfoTab').style.display)
        return !flag;
    }
    ...
    if (control == flag) {
        ...
        await page.evaluate(() => document.getElementsByTagName('frame')[1].contentWindow.document.getElementById('wlanac_enbl').getElementsByTagName('input')[0].click());
        await page.waitForTimeout(1000);

        await page.evaluate(() => document.getElementsByTagName('frame')[1].contentWindow.document.getElementById('btnOK').click());
        await page.waitForTimeout(1000);

        console.log(`${new Date()}: 成功${status}wifi 5g!`)
    }
